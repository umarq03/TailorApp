...
export class KurtaPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.wkurta = this.cartService.getwkurta();
  ...
  removeItem(product) {
    this.cartService.removekurta(product);
  }
  removedesigns(product) {
    this.cartService.removekurtadesigns(product);
  }
  getTotal() {
  ...
  async addDatabase() {
    if (this.idea.fullname == "") {
      ...
      this.showToast('<ion-icon name="warning-outline"></ion-icon> Shalwar is not select!')
    }
  ...
  async deleteIdea() {
    ...
    this.database.deleteKurta(this.idea.id).then(() => {
      ...
      loading.dismiss();
      this.showToast1('<ion-icon name="checkmark-outline"></ion-icon> Kurtadata deleted');
    }, err => {
  ...
  async updateIdea() {
    ...
    this.database.updateKurta(this.idea).then(() => {
      this.router.navigateByUrl('/admin-database')
      loading.dismiss();
      this.showToast1('<ion-icon name="checkmark-outline"></ion-icon> Kurtadata updated');
    }, err => {
  ...
  sendSMS() {
    ...
    this.sms.send('03018761467', this.idea.fullname + ' ' + 'is send you kurta data at TailorShop.' + ' ' + 'customer phonenumber is :' + ' ' + this.idea.phonenumber + ' ' + 'at' + this.idea.creaditAt, options).then(() => {
  ...
  send() {
    ...
    this.sms.send(String(this.idea.phonenumber), this.idea.fullname + ' ' + 'your kurta data successfully sent, after 10 minutes we will call/sms you for confirmation , from TailorShop!', options).then(() => {
