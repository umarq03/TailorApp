...
@Component({
  selector: 'app-coatdesign',
  templateUrl: './coatdesign.page.html',
  styleUrls: ['./coatdesign.page.scss'],
})
export class CoatdesignPage implements OnInit {
  selectedFile: any;
  ...
  async uploadFile(id, file): Promise<any> {
    if (file && file.length) {
      try {
        const task = await this.storage.ref('coatdesign').child(id).put(file[0]);
        return this.storage.ref(`coatdesign/${id}`).getDownloadURL().toPromise();
      } catch (err) {
        console.log(err);
      }
    }
