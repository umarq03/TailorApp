...
@Component({
  selector: 'app-editshirtfabric',
  templateUrl: './editshirtfabric.page.html',
  styleUrls: ['./editshirtfabric.page.scss'],
})
export class EditshirtfabricPage implements OnInit {
  ...
  async updateSummer(id, fabname, price, discount, stock, meter) {
    ...
    this.shirtsummerfab.doc(this.fabric.id).update({
  ...
  async updateWinter(id, fabname, price, discount, stock, meter) {
    ...
    await loading.present();
    this.shirtwinterfab.doc(this.fabric.id).update({
      fabname: this.fabric.fabname,
      price: this.fabric.price,
      discount: this.fabric.discount,
      stock: this.fabric.stock,
