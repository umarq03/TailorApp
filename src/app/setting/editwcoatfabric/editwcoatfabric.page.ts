...
@Component({
  selector: 'app-editwcoatfabric',
  templateUrl: './editwcoatfabric.page.html',
  styleUrls: ['./editwcoatfabric.page.scss'],
})
export class EditwcoatfabricPage implements OnInit {
  ...
  async updateSummer(id, fabname, price, discount, stock, meter) {
    ...
    this.wcoatsummerfab.doc(this.fabric.id).update({
  ...
  async updateWinter(id, fabname, price, discount, stock, meter) {
    ...
    await loading.present();
    this.wcoatwinterfab.doc(this.fabric.id).update({
      fabname: this.fabric.fabname,
      price: this.fabric.price,
      discount: this.fabric.discount,
      stock: this.fabric.stock,
