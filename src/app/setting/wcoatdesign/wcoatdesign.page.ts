...
@Component({
  selector: 'app-wcoatdesign',
  templateUrl: './wcoatdesign.page.html',
  styleUrls: ['./wcoatdesign.page.scss'],
})
export class WcoatdesignPage implements OnInit {
  selectedFile: any;
  ...
  async uploadFile(id, file): Promise<any> {
    if (file && file.length) {
      try {
        const task = await this.storage.ref('wcoatdesign').child(id).put(file[0]);
        return this.storage.ref(`wcoatdesign/${id}`).getDownloadURL().toPromise();
      } catch (err) {
        console.log(err);
      }
    }
