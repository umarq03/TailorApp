...
constructor(private database: DatabaseService, private activateR: ActivatedRoute){
}
...
ngOnInit() {
  ...
  addPantfab(product) {
    let added = false;
    for (let pantfab of this.pantfab) {

      if (pantfab.id === product.id) {
        pantfab.amount += 1;
        added = true;
        break;
      }
    ...
    if (!added) {
      this.pantfab.push(product);
    }
  }
  addShirtfab(product) {
    let added = false;
    for (let shirtfab of this.shirtfab) {

      if (shirtfab.id === product.id) {
        shirtfab.amount += 1;
        added = true;
        break;
      }
  ...
  addKurtafab(product) {
    ...
    if (!added) {
      this.kurtafab.push(product);
    }
  }
  addCoatfab(product) {
    let added = false;
    for (let coatfab of this.coatfab) {

      if (coatfab.id === product.id) {
        coatfab.amount += 1;
        added = true;
        break;
      }
    ...
    if (!added) {
      this.coatfab.push(product);
    }
  }
  addWaistcoatfab(product) {
    let added = false;
    for (let waistcoatfab of this.waistcoatfab) {

      if (waistcoatfab.id === product.id) {
        waistcoatfab.amount += 1;
        added = true;
        break;
      }
    }
    if (!added) {
      this.waistcoatfab.push(product);
    }
  }
